...
try {
    if (-not (Test-Path $sourceBroker)) {
        throw "Published Broker directory was not found: $sourceBroker"
    }
    ...
    New-Service `
        ...
        -DisplayName 'SplitOS Broker CI Smoke' `
    ...
    if ($service.StartMode -ne 'Auto') {
        throw "Broker service StartMode must be Auto, got $($service.StartMode)."
    }

    if ($service.StartName -notin @('LocalSystem', 'LocalSystem ')) {
        throw "Broker service must run as LocalSystem, got '$($service.StartName)'."
    }

    if (-not $service.ProcessId) {
        throw 'Broker service has no process id.'
    }

    $process = Get-CimInstance Win32_Process -Filter "ProcessId=$($service.ProcessId)"
    if (-not $process) {
        throw 'Broker process could not be resolved through Win32_Process.'
    }

    if ($process.SessionId -ne 0) {
        throw "Broker must execute in service session 0, got session $($process.SessionId)."
    }
    ...
    Write-Host 'Broker Windows Service smoke passed: LocalSystem, Automatic, Session 0, exact published image.'
}
finally {
    Stop-Service -Name $serviceName -Force -ErrorAction SilentlyContinue
