...
public sealed class RuntimeSignOutClient(string componentName, string componentVersion)
{
    public async Task<RuntimeSignOutResult> SignOutAsync(CancellationToken cancellationToken = default)
    {
        ...
        var request = WireMessage.Create(
            MessageTypes.RuntimeSignOutRequest,
            new RuntimeSignOutRequest(),
            Capabilities.RuntimeSignOut);
        ...
        if (!string.Equals(response.MessageType, MessageTypes.RuntimeSignOutResult, StringComparison.Ordinal))
        {
            throw new InvalidDataException($"Unexpected Runtime response type {response.MessageType}.");
        }

        return response.ReadPayload<RuntimeSignOutResult>();
    }
}
