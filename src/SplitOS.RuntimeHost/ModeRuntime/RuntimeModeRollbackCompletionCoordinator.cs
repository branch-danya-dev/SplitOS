...
public sealed class RuntimeModeRollbackCompletionCoordinator(
    IModeTransitionStore transitions, IModeTransitionReconciliationStore reconciliation,
    IModeSourceAuthority authority, IManagedServiceSourceVerificationClient broker)
{
    public async Task<RuntimeModeRollbackStepOutcome> CompleteAsync(Guid transitionId, CancellationToken cancellationToken = default)
    {
        ...
        var authorization = await authority.EvaluateAsync(transition, cancellationToken).ConfigureAwait(false);
        if (!authorization.MayRestoreSource) return new("RECONCILIATION_REQUIRED", authorization.ProductCode);
        var verified = await broker.VerifySourceAsync(transition.OperationId, transition.CorrelationId,
            new(transitionId, transition.LeaseId, transition.FenceToken, transition.ControlSessionKey, transition.Revision), cancellationToken).ConfigureAwait(false);
        if (verified.Disposition != "VERIFIED") return new(verified.Disposition, verified.ProductCode);
        authorization = await authority.EvaluateAsync(transition, cancellationToken).ConfigureAwait(false);
        if (!authorization.MayRestoreSource) return new("RECONCILIATION_REQUIRED", authorization.ProductCode);
        var stage = await transitions.AdvanceAsync(transitionId, transition.Revision, transition.LeaseId, transition.FenceToken,
        ...
        authorization = await authority.EvaluateAsync(stage.Transition, cancellationToken).ConfigureAwait(false);
        if (!authorization.MayRestoreSource) return new("RECONCILIATION_REQUIRED", authorization.ProductCode);
        var terminal = await transitions.AdvanceAsync(transitionId, stage.Transition.Revision, transition.LeaseId, transition.FenceToken,
