...
public enum ModeVerificationEvidenceDisposition
{
    Authorized,
    Missing,
    RevisionConflict,
...
public sealed record ModeVerificationEvidenceOutcome(
    ModeVerificationEvidenceDisposition Disposition,
    string ProductCode,
    int? ActualActionRevision = null,
    string? Detail = null)
{
    public bool IsAuthorized => Disposition == ModeVerificationEvidenceDisposition.Authorized;
}

/// <summary>
/// Read-side authorization for semantic target verification after BeginVerify. This boundary never
/// authorizes mutation: it requires a current MODE lease/fence, VERIFYING/VERIFY_STARTED transition,
/// an exact immutable action binding and a durable action that is already VERIFYING after APPLIED.
/// </summary>
public sealed class ModeVerificationEvidenceStore
{
    ...
    public ModeVerificationEvidenceStore(
    ...
    public async Task<ModeVerificationEvidenceOutcome> ValidateAsync(
        ModeMutationFenceContext context,
        CancellationToken cancellationToken = default)
    {
        ValidateContext(context);
        if (context.ExpectedAction is null)
            throw new ArgumentException("Verification evidence requires immutable action semantics.", nameof(context));
        EnsureCanonicalStoreAvailable();
        ...
        if (lease is null || !lease.LeaseId.HasValue)
            return Denied(ModeVerificationEvidenceDisposition.LeaseConflict, "MUTATION_LEASE_STALE_FENCE", "No active major mutation lease exists.");

        if (_timeProvider.GetUtcNow() >= lease.ExpiresUtc)
            return Denied(ModeVerificationEvidenceDisposition.ReconciliationRequired, "MUTATION_LEASE_RECONCILIATION_REQUIRED", "Major mutation lease expired before verification evidence was read.");
        ...
        {
            return Denied(ModeVerificationEvidenceDisposition.LeaseConflict, "MUTATION_LEASE_STALE_FENCE", "Current MODE lease/fence does not match the verification request.");
        }
        ...
        if (transition is null)
            return Denied(ModeVerificationEvidenceDisposition.Missing, "MODE_VERIFY_TRANSITION_NOT_FOUND", "Owning mode transition does not exist.");
        ...
        {
            return Denied(ModeVerificationEvidenceDisposition.OwnershipConflict, "MODE_VERIFY_TRANSITION_MISMATCH", "Transition ownership no longer matches the verification request.");
        }

        if (!string.Equals(transition.State, "VERIFYING", StringComparison.Ordinal) ||
            !string.Equals(transition.Stage, "VERIFY_STARTED", StringComparison.Ordinal) ||
            transition.CommitDurable)
        {
            return Denied(ModeVerificationEvidenceDisposition.InvalidLifecycle, "MODE_VERIFY_INVALID_LIFECYCLE", "Verification evidence may be read only during VERIFYING/VERIFY_STARTED before durable commit.");
        }

        var action = await ReadActionAsync(connection, transaction, context.ActionId, cancellationToken).ConfigureAwait(false);
        if (action is null)
            return Denied(ModeVerificationEvidenceDisposition.Missing, "MODE_VERIFY_ACTION_NOT_FOUND", "Owning durable action does not exist.");

        if (action.TransitionId != context.TransitionId)
            return Denied(ModeVerificationEvidenceDisposition.OwnershipConflict, "MODE_VERIFY_ACTION_MISMATCH", "Durable action belongs to a different transition.", action.Revision);

        if (action.Revision != context.ExpectedActionRevision)
            return Denied(ModeVerificationEvidenceDisposition.RevisionConflict, "MODE_ACTION_REVISION_CONFLICT", $"Expected action revision {context.ExpectedActionRevision}, actual {action.Revision}.", action.Revision);

        if (!action.PlanComplete)
            return Denied(ModeVerificationEvidenceDisposition.InvalidLifecycle, "MODE_ACTION_PLAN_INCOMPLETE", "Durable action-plan header/count no longer matches the action journal.", action.Revision);

        if (!MatchesActionBinding(action, context.ExpectedAction))
            return Denied(ModeVerificationEvidenceDisposition.InvalidLifecycle, "MODE_VERIFY_ACTION_SEMANTICS_MISMATCH", "Verification request does not match immutable durable action semantics.", action.Revision);

        if (!string.Equals(action.State, "VERIFYING", StringComparison.Ordinal) ||
            !string.Equals(action.ApplyResultCode, "APPLIED", StringComparison.Ordinal) ||
            action.AppliedUtc is null ||
            action.VerifyResultCode is not null)
        {
            return Denied(ModeVerificationEvidenceDisposition.InvalidLifecycle, "MODE_VERIFY_ACTION_NOT_READY", "Verification evidence requires a VERIFYING action with durable APPLIED evidence and no prior verification result.", action.Revision);
        }

        transaction.Commit();
        return new ModeVerificationEvidenceOutcome(
            ModeVerificationEvidenceDisposition.Authorized,
            "MODE_VERIFY_AUTHORIZED",
            action.Revision);
    }
    ...
    private async Task<SqliteConnection> OpenReadyAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (version != MachineStateStore.SchemaVersion)
                throw new InvalidDataException($"Verification evidence requires machine schema {MachineStateStore.SchemaVersion}, got {version}.");
    ...
    private void EnsureCanonicalStoreAvailable()
    {
        ...
        if (!File.Exists(_markerPath) || !File.Exists(_databasePath))
            throw new InvalidDataException("Machine canonical store must be initialized before verification evidence is evaluated.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (reader.IsDBNull(0))
            return new LeaseRow(null, null, null, null, null, reader.GetInt64(5), DateTimeOffset.MinValue);
        ...
        {
            throw new InvalidDataException("Major mutation lease contains malformed verification values.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException("Mode transition contains malformed verification ownership values.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            SELECT action_row.transition_id, action_row.action_state, action_row.revision,
                   ...
                   action_row.apply_result_code, action_row.applied_utc, action_row.verify_result_code,
        ...
        if (!Guid.TryParse(reader.GetString(0), out var transitionId))
            throw new InvalidDataException("Mode action contains malformed verification transition ownership.");

        DateTimeOffset? appliedUtc = null;
        if (!reader.IsDBNull(9))
        {
            if (!DateTimeOffset.TryParse(reader.GetString(9), out var parsed))
                throw new InvalidDataException("Mode action contains malformed applied timestamp.");
            appliedUtc = parsed.ToUniversalTime();
        }
    ...
    private static void ValidateContext(ModeMutationFenceContext context)
    {
        ...
        if (context.TransitionId == Guid.Empty ||
            context.ActionId == Guid.Empty ||
            context.LeaseId == Guid.Empty ||
            context.OperationId == Guid.Empty ||
            context.CorrelationId == Guid.Empty)
        {
            throw new ArgumentException("Verification context identifiers must not be empty.", nameof(context));
        }
        ...
        if (string.IsNullOrWhiteSpace(context.ControlSessionKey) ||
            ...
            context.ControlSessionKey.Any(static c => char.IsControl(c)))
        {
            throw new ArgumentException("ControlSessionKey is outside supported bounds.", nameof(context));
        }
    }

    private static ModeVerificationEvidenceOutcome Denied(
        ModeVerificationEvidenceDisposition disposition,
        string code,
        string detail,
        int? revision = null)
